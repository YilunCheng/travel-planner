...
一个**完全本地、单用户**的旅行行程规划器，跑在自己的 Mac 上。用一个单文件网页 UI **逐日查看行程、浏览每趟的文档、就地编辑与拖拽排版（含地图、天气、航班实时信息，以及一个自动带上行程上下文的 💬 AI 聊天框）**；也能把 `~/Documents/Travel Plan/` 里的旧行程文档导入成结构化 JSON。`data/` 下的结构化 JSON 是 going-forward 的**唯一真源**——不再需要用 Apple Pages 编辑行程。
...
3. **手机交互对照**：桌面的悬停操作在手机上收进每行/每天/标题旁的 **⋯ 底部菜单**；**长按拖拽**排序；右下 **🗺 悬浮球**滑出全屏地图、**💬 悬浮球**（叠在 🗺 上方）滑出行程聊天；文档点开在浏览器内 QuickLook 预览；**导出 PDF 由 Mac 上的无头 Chrome 代为渲染**（Mac 装有 Chrome/Chromium/Edge/Brave 任一即可；一台都没有则该入口自动隐藏）。
...
**核心体验不变。** 新建/编辑行程、地图搜索与手动定位、天气（气候 + 逐日预报）、航班解析与实时信息、文档、PDF 导出、手机端——这些**全都不经过 LLM**。`claude` CLI 只在五处使用，且全部优雅降级：
...
| **💬 行程聊天**（行程页右下角聊天框） | 自动带上行程/天气/文档上下文的讨论助手，可选模型与 effort | 聊天球自动隐藏；其余一切照常 |
...
唯一的 LLM 是本地 `claude` CLI。三处在**导入/生成阶段**做"杂乱文本 → 干净结构化知识"（封装 `import/structure.py:ask_claude()`），另有两处**浏览时**调用：✨ 天气摘要（封装 `server.py:_ask_claude_text()`，经 `/api/weather/summary` 触发、按 prompt 哈希缓存）和 💬 行程聊天（`server.py:chat()`，经 `/api/chat` **流式**返回）。前两类封装跑的是同一条命令：
...
- 💬 聊天走的命令略不同：`--output-format stream-json --include-partial-messages`（打字机流式）；**模型与 effort 由聊天框里的下拉选择**（默认 = `CLAUDE_MODEL` / CLI 默认）；**不带 `bypassPermissions` 且禁用全部工具**、工作目录为空目录——纯文本问答，碰不到文件与网络；对话式，不缓存。
...
- 前三处按行程导入/生成时跑一次；第 4 处虽由浏览触发，但**有缓存**（同一行程同样的天气只生成一次），且失败时静默降级为普通天气卡；第 5 处是对话，**只在你主动发消息时**才调用（每条消息一次）。
